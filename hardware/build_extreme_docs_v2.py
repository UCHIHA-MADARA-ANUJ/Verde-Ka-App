...
# ----------------------------------------------------
# 00_Project_Overview.md (Extreme Detail)
# ----------------------------------------------------
with open("App/00_Project_Overview.md", "w") as f:
    f.write("""# 00. Project Overview & Comprehensive Specification

This document provides a highly detailed, professional-grade specification of **Project Verde V3.0 (Autonomous Plant OS)**. It is written to serve as the definitive introduction for developers, engineers, and evaluators who have zero prior knowledge of the project.

---

## 🎯 1. Project Goal & Business Value
Project Verde V3.0 is designed to bridge the gap between amateur home gardening and industrial-grade precision farming. The system is engineered to solve a common, real-world problem: **how to maintain optimal plant health and hydration when the caretaker is far away, without relying on unstable internet connections or high-maintenance infrastructure.**

### Primary Value Pillars:
1. **Water Conservation:** Uses real-time soil moisture sensors and predictive weather forecasting to apply only the precise amount of water required, preventing overwatering and run-off.
2. **Hardware Autonomy:** The edge node is a self-contained autonomous unit. It operates its watering loops locally and falls back to pre-set parameters if the cloud connection is lost.
3. **Pest and Disease Prevention:** Integrates standalone computer vision (ESP32-CAM) to identify leaf pathogens on-demand, providing early warnings before infections spread.
4. **Low Cost & Ease of Build:** Built entirely using readily available consumer-grade components mounted on a single breakout expansion shield, cutting costs by 75% compared to industrial PLC-based agricultural monitors.

---

## 👥 2. Target Users & Expected Audits
* **Urban & Home Gardeners:** Individuals who travel frequently and require a set-and-forget remote irrigation system.
...
* **DHT22 Sensor (Ambient Temp/Hum):** Measures ambient parameters with decimal precision.
* **2-Prong Moisture Sensor:** Measures volumetric water content in the soil.
* **HC-SR04 Ultrasonic Sensor:** Measures water level inside the bucket reservoir.
* **LDR Light Sensor:** Detects ambient light levels to determine grow light state.
* **5V Submersible Pump:** Low-power 5V pump that can lift water up to 1 meter.
* **Everlight 5mm UV LED:** Low-power ultraviolet light emitting diode used as a local photosynthetic grow light.
...
|                                                                                                   |
|  +──────────────────────────────────────────+     (HTTPS)     +────────────────────────────────+  |
|  |             Next.js Frontend             |  ◄============► |  Vercel Serverless API Routes  |  |
|  |     - Tailwind CSS Custom Theme          |                 |  - /api/upload-photo (Node.js) |  |
|  |     - Recharts Live Moisture Line        |                 |  - /api/analyze-plant (Edge)   |  |
|  |     - Animated Radial Dials & Cylinder   |                 |  - /api/weather-sync (Edge)    |  |
|  +──────────────────────────────────────────+                 +────────────────────────────────+  |
|                        ▲                                                      │                   |
|                        │                                                      │                   |
...
|                        ▼                                                      ▼                   |
|  +──────────────────────────────────────────+                 +────────────────────────────────+  |
|  |       Firebase Realtime DB (RTDB)        |                 |        Firebase Storage        |  |
|  |  - Telemetry and Actuator State Trees    |                 |   - 5GB Free Storage Bucket    |  |
|  |  - Open WebSocket Streams                |                 |   - Folder: /scans/{userId}/   |  |
|  +──────────────────────────────────────────+                 +────────────────────────────────+  |
|                        ▲                                                      ▲                   |
|                        │                                                      │                   |
|                   (WiFi Sync)                                            (HTTP POST)              |
...
|  +──────────────────────────────────────────+                                 │                   |
|  |          ESP32 Main Brain                |                                 │                   |
|  |     (DOIT DevKit V1 on GVS Shield)       |                                 │                   |
|  |  - Gated 2-Prong Moisture (GPIO 23/34)   |                                 │                   |
|  |  - DHT22 (GPIO 4)                        |                                 │                   |
|  |  - HC-SR04 Trigger/Echo (GPIO 18/19)     |                                 │                   |
|  |  - LDR Analog Light (GPIO 35)            |                                 │                   |
|  |  - Robocraze 5V Pump Relay (GPIO 25)     |                                 │                   |
|  |  - Direct 5mm UV LED (GPIO 26)           |                                 │                   |
|  +──────────────────────────────────────────+                                 │                   |
|                                                                               │                   |
|  +──────────────────────────────────────────+                                 │                   |
|  |         ESP32-CAM Node                   | ────────────────────────────────┘                   |
|  |     (Standalone AI Camera)               |                                                     |
|  |  - Snaps high-res JPEG only on request   |                                                     |
|  |  - Throttled 8MHz XCLK (RF Interference) |                                                     |
|  +──────────────────────────────────────────+                                                     |
+───────────────────────────────────────────────────────────────────────────────────────────────────+
```
""")
...
# ----------------------------------------------------
# 01_System_Architecture.md (Extreme Detail)
# ----------------------------------------------------
...
### D. Sensors & Actuators
1. **DHT22/AM2302 (Ambient Temp/Humidity):** High-precision sensor using a single-bus digital interface.
2. **2-Prong Soil Moisture Probe:** Analog resistive sensor. Connects its VCC to **GPIO 23** (power-gating pin) to prevent electrolysis and rust.
3. **HC-SR04 (Ultrasonic Distance):** Emits a 40kHz sonar pulse to measure water height in the bucket.
4. **LDR Sensor:** Light-dependent resistor. Outputs analog values which are mapped to determine grow light state.
5. **Robocraze 5V Dual-Channel Relay Module:** Features optocoupler isolation, protecting your ESP32 pins from high back-EMF spikes when the pump turns on.
6. **5V Submersible Pump:** Brushless DC motor pump capable of lifting water up to 1 meter through silicone tubing.
7. **Everlight 5mm UV LED:** Purple ultraviolet LED emitting 395nm wavelength to promote plant leaf growth.
...
* **Role:** Lightweight, global API routes that handle receiving binary image buffers from the ESP32-CAM, saving them to storage, fetching OpenWeatherMap Delhi rain forecasts, and handshaking with Plant.id and Gemini 2.0 Flash.
""")

# ----------------------------------------------------
# 02_Application_Workflow.md (Extreme Detail)
# ----------------------------------------------------
with open("App/02_Application_Workflow.md", "w") as f:
    f.write("""# 02. Application Workflow & Event Sequences
...
User App                  Firebase RTDB                 ESP32 Brain             ESP32-CAM
  ...
  │                            │                             │                       │
  │──(5. Click "Scan Leaf")───►│(capture_photo = true)       │                       │
  │                            │                             │                       │
  ...
  │◄──(8. POSTs raw JPEG)──────┼─────────────────────────────┼───────────────────────│
  │                            │                             │                       │
  │──(9. Reset capture flag)──►│(capture_photo = false)      │                       │
  │                            │                             │                       │
  │──(10. Plant.id & Gemini)──►│                             │                       │
  │                            │                             │                       │
...
* **Action:** Next.js opens a permanent WebSocket stream to the `/sensors` and `/controls` paths in Firebase RTDB.
* **Data Format:** WebSocket frames containing NoSQL JSON trees.
* **Telemetry Upload (ESP32):** The main **ESP32 DevKit** board reads its local sensors and writes a JSON payload to Firebase every 4 seconds.
* **JSON Payload Format:**
  ```json
  {
    "moisture": 52,
...
### Step 3: Triggering a Parameter (Auto Threshold Slider)
* **Action:** The user slides the "Soil Moisture Target" slider on the dashboard to **45%**.
* **Database Write:** The Next.js app immediately writes `moisture_threshold = 45` to the `/controls` node of Firebase RTDB.
...
* **Device Actuation:**
  ...
  2. Because the soil is dry, the ESP32 pulls **PUMP_RELAY LOW (ON)**.
  3. The ESP32 writes `/controls/pump_state = true` to Firebase, which instantly animates the pump icon on the user's screen.
...
# ----------------------------------------------------
# 03_ESP32_Communication.md (Extreme Detail)
# ----------------------------------------------------
with open("App/03_ESP32_Communication.md", "w") as f:
    f.write("""# 03. ESP32 Communication Protocol

This document details the telemetry formats, controls structure, and Wi-Fi reconnection architectures of the **ESP32 DevKit V1** main controller.
...
# ----------------------------------------------------
# 04_ESP32_CAM_Workflow.md (Extreme Detail)
# ----------------------------------------------------
...
# ----------------------------------------------------
# 05_Backend_Architecture.md (Extreme Detail)
# ----------------------------------------------------
...
* **Logic:** Triggered via a Vercel cron schedule once an hour. Fetches current weather for Delhi (`LAT: 28.6139° N, LON: 77.209° E`) from the **OpenWeatherMap API**. 
  * If `"rain"`, `"drizzle"`, or `"thunderstorm"` is found, writes `weather_override = 1` in Firebase.
  * If clear, writes `weather_override = 0`.
""")

# ----------------------------------------------------
# 06_Supabase_vs_Firebase.md (Extreme Detail)
# ----------------------------------------------------
...
# ----------------------------------------------------
# 07_Database_Design.md (Extreme Detail)
# ----------------------------------------------------
...
# ----------------------------------------------------
# 08_API_Design.md (Extreme Detail)
# ----------------------------------------------------
...
# ----------------------------------------------------
# 09_Authentication.md (Extreme Detail)
# ----------------------------------------------------
...
# ----------------------------------------------------
# 10_Row_Level_Security.md (Extreme Detail)
# ----------------------------------------------------
...
# ----------------------------------------------------
# 11_Image_Handling.md (Extreme Detail)
# ----------------------------------------------------
...
# ----------------------------------------------------
# 12_Offline_and_Error_Handling.md (Extreme Detail)
# ----------------------------------------------------
...
# ----------------------------------------------------
# 13_Notifications.md (Extreme Detail)
# ----------------------------------------------------
...
# ----------------------------------------------------
# 14_UI_UX_Guidelines.md (Extreme Detail)
# ----------------------------------------------------
...
# ----------------------------------------------------
# 15_Project_Rules.md (Extreme Detail)
# ----------------------------------------------------
...
# ----------------------------------------------------
# 16_Best_Practices.md (Extreme Detail)
# ----------------------------------------------------
...
# ----------------------------------------------------
# 17_Future_Improvements.md (Extreme Detail)
# ----------------------------------------------------
...
# ----------------------------------------------------
# 18_Development_Checklist.md (Extreme Detail)
# ----------------------------------------------------
...
print("Extreme Book-Quality Documentation completed successfully on disk!")
